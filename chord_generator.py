...
def main():
    path = '/Users/benshi/Documents/BillboardProject/ChordTransformer/models/chord_generator_POP909'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = torch.load(path, device).to(device)
    chord_generator = ChordGenerator(model, device)
    temperature = 0.8

    context_seq = 'Start-1M'
    ...
    output = chord_generator.generate_all_k_sampling(context_seq_idx, temperature)
    print(output)

class ChordGenerator:
    def __init__(self, model, device):
        self.model = model
        self.max_len = 222 # Set as constant for now
        self.src_mask = generate_square_subsequent_mask(self.max_len).to(device)
        self.softmax = nn.Softmax()
    
    # context_seq is array of numbers
